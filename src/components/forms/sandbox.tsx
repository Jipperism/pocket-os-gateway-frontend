...
export const SandboxForm = ({
	...
	// Use props if provided, otherwise use first available option
	const initialChain = propChain ?? networks?.[0]?.value ?? "";
	const initialApplication =
		propApplication ?? applications?.[0]?.portalApplicationId ?? "";
	...
	const form = useAppForm({
		...
		defaultValues: {
			...
			chain: initialChain,
			endpointUrl: getEndpointUrl(initialChain, initialApplication),
			method: "POST",
			path: "/",
 ...
 "jsonrpc": "2.0"
		...
		onSubmit: async ({ value }) => {
			// perform request
			const response = await fetch(value.endpointUrl, {
				method: value.method,
				body: value.body,
